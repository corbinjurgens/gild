...
// ThreadSafeRepository once and reuses it. Uses imara-diff internally
// for faster blame than libgit2's diff engine.
thread_local! {
    static THREAD_REPO: RefCell<Option<gix::Repository>> = RefCell::new(None);
}
...
pub fn compute(
    ...
    let cache_key = format!("{}{}", CACHE_PREFIX, head.id());
    ...
    let oid_to_email: HashMap<String, String> = {
        let mut stmt = db
            .conn
            .prepare("SELECT hash, LOWER(author_email) FROM commits")?;
    ...
    let commits_complete = {
        let head_in_table: bool = db
            ...
            .query_row(
                ...
                params![head.id().to_string()],
                |_| Ok(()),
            )
            .is_ok();
        let mut walk = repo.revwalk()?;
        walk.push(head.id())?;
        let reachable = walk.count() as i64;
        let in_table: i64 =
            db.conn
    ...
    // every line to them anyway. Skip the (slow) libgit2 blame and write
    ...
    let already_done: HashSet<String> = {
        let mut stmt = db.conn.prepare(
            "SELECT DISTINCT file_path FROM blame_file WHERE head_hash = ?1",
        )?;
    ...
    let gix_path = repo.workdir().unwrap_or_else(|| repo.path());
    let safe_repo = gix::ThreadSafeRepository::open(gix_path)?;
    let gix_head_oid = gix::ObjectId::from_hex(head.id().to_string().as_bytes())?;
        ...
        let results: Vec<(String, HashMap<String, usize>)> = chunk
            ...
            .map(|file_path| {
                let local = THREAD_REPO.with(|cell| {
                    ...
                    if opt.is_none() {
                        *opt = Some(safe_repo.to_thread_local());
                    }
                    match opt.as_ref() {
...
fn blame_file_lines(
    ...
    let outcome = match repo.blame_file(
...
pub fn walk_tree_sizes(repo: &Repository, tree: &git2::Tree) -> HashMap<String, usize> {
